...
import { LocationNode, DEFAULT_NODES, getLocationNodes, saveLocationNodes } from '@/components/LocationSelector';
...
export default function LocationHierarchyPage() {
  ...
  const saveLocationState = (updated: LocationNode[]) => {
    setNodes(updated);
    saveLocationNodes(updated as any);
  };

  const handleResetDefaults = () => {
    if (confirm('Reset all location hierarchy data to system production defaults?')) {
      saveLocationNodes(DEFAULT_NODES as any);
      setNodes(DEFAULT_NODES);
      logAudit('Reset Location Hierarchy', nodes, DEFAULT_NODES, 'Reset location hierarchy to system production defaults');
      alert('Location hierarchy successfully reset to system defaults!');
    }
  };
  ...
  const handleToggleStatus = (node: LocationNode) => {
    const newStatus: 'Active' | 'Inactive' = node.status === 'Active' ? 'Inactive' : 'Active';
    const updated: LocationNode[] = nodes.map(n => {
      if (n.id === node.id) {
  ...
  return (
    <AdminGuard pageTitle="Location Hierarchy">
      <div className="admin-header-bar glass" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px', borderBottom: '1px solid var(--border-color)', background: 'var(--bg-card)' }}>
        ...
        <button onClick={handleResetDefaults} className="btn btn-secondary" style={{ padding: '8px 16px', borderRadius: '6px', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '6px', border: '1px solid #fca5a5', color: '#dc2626', background: '#fef2f2' }} title="Reset all location hierarchy to production defaults">
